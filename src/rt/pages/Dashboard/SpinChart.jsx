...
function SpinChart() {
  ...
  const fetchCounts = useCallback(async () => {
    ...
    try {
      ...
      // unique KK
      const kkSet = new Set((pendudukRes.data || []).map(r => r.no_kk).filter(Boolean));
      const totalKK = kkSet.size;

      // gender split
      let perempuan = 0;
      let laki = 0;
      (pendudukRes.data || []).forEach(r => {
        const jk = String(r.jk || "").toLowerCase();
        if (jk.startsWith("p")) perempuan++;
        else if (jk.startsWith("l")) laki++;
      });

      // 2) totals for other tables, filtered by RT/RW
      const [kelahiranRes, kematianRes, pindahRes, pendatangRes] = await Promise.all([
        supabase.from("data_kelahiran").select("id_kelahiran", { count: "exact" }).eq("rt", rt).eq("rw", rw),
        supabase.from("data_kematian").select("id", { count: "exact" }).eq("rt", rt).eq("rw", rw),
        supabase.from("data_pindah").select("id", { count: "exact" }).eq("rt", rt).eq("rw", rw),
        supabase.from("data_pendatang").select("id", { count: "exact" }).eq("rt", rt).eq("rw", rw),
      ]);
  ...
  return (
    <div className="bg-white rounded-2xl shadow-md border border-slate-200 p-6 flex flex-col h-[480px]">
      <div className="mb-4">
        ...
        <p className="text-sm text-slate-500">Distribusi data (Realtime, sesuai RT/RW login)</p>
      </div>
