// src/rt/pages/RWEditAkun.jsx
import React, { useState, useEffect, useRef } from "react";
import { useParams, useNavigate } from "react-router-dom";
import { Eye, EyeOff, ArrowLeft } from "lucide-react";
import supabase from "../../../supabaseClient";
import defaultAvatar from "../../../assets/logo.png"; // pastikan file ada
    ...
    const fetchUser = async () => {
      ...
      try {
        ...
        if (data?.foto) {
          ...
          // no foto in DB => use default avatar
          setFotoPreview(defaultAvatar);
        }
      } catch (err) {
      ...
      <form onSubmit={handleUpdate} className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="text" value={formData.rw} onChange={(e) => setFormData({ ...formData, rw: e.target.value })} className="w-full border rounded px-3 py-2 bg-gray-100 text-gray-700 cursor-not-allowed" disabled/>
          </div>
