// src/rw/pages/sirkulasipenduduk/DetailPendatangPerRT.jsx
import { useState, useEffect } from "react";
import { FileText, Eye, ArrowLeft } from "lucide-react";
import supabase from "../../../../supabaseClient";
import { useLocation, useParams, Link, useNavigate } from "react-router-dom";

function DetailPendatangPerRT() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        let query = supabase.from("data_pendatang").select("*");
        ...
        if (!mounted) return;
        if (error) {
          console.error("Error fetching data pendatang:", error);
          setDataPendatang([]);
        } else {
          setDataPendatang(data || []);
          console.log(
            `Fetched ${data?.length || 0} pendatang (rtParam=${rtParam}, keyword=${keyword})`
          );
        }
      } catch (err) {
        console.error("Unexpected error:", err);
        if (mounted) setDataPendatang([]);
      } finally {
  ...
  }, [location.search, rtParam, userRw]); // ⬅️ tambahkan userRw

  // Pagination
  const totalPages = Math.max(1, Math.ceil(dataPendatang.length / entriesPerPage));
  const indexOfLast = currentPage * entriesPerPage;
  const indexOfFirst = indexOfLast - entriesPerPage;
  const currentData = dataPendatang.slice(indexOfFirst, indexOfLast);
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow relative">
      ...
      <div className="overflow-x-auto mt-4">
        <table className="min-w-full border border-gray-200 text-center table-auto">
            ...
            {loading ? (
              ...
              currentData.map((item, index) => (
                <tr key={item.id} className="hover:bg-gray-50">
                  <td className="px-2 py-1 border">{indexOfFirst + index + 1}</td>
                  <td className="px-2 py-1 border">{item.nik}</td>
                  <td className="px-2 py-1 border">{item.no_kk}</td>
                  <td className="px-2 py-1 border">{item.nama}</td>
                  <td className="px-2 py-1 border">{item.jk}</td>
                  <td className="px-2 py-1 border">{item.tanggal_datang}</td>
                  <td className="px-2 py-1 border">
                    <div className="flex justify-center items-center">
                      <Link
                        to={`/rw/rwsirkulasipenduduk/detaildatapendatangrt/${item.id}`}
                        className="p-1 text-blue-600 hover:bg-blue-100 rounded-full flex justify-center items-center"
                      >
      ...
      <div className="flex justify-between items-center mt-4">
        <span className="text-sm">
          Showing {dataPendatang.length === 0 ? 0 : indexOfFirst + 1} to{" "}
          {Math.min(indexOfLast, dataPendatang.length)} of {dataPendatang.length} entries
        </span>
...
export default DetailPendatangPerRT;
