...
async function fetchAllRows(table, columns = "*", rw = null, chunkSize = 1000) {
  ...
  while (true) {
    ...
    if (rw) query = query.eq("rw", rw); // filter per RW
...
function SpinChart() {
  ...
  const fetchCounts = useCallback(async () => {
    ...
    try {
      ...
      const pendudukRows = await fetchAllRows("data_penduduk", "nik, no_kk, jk", userRw);
      const totalPenduduk = pendudukRows.length;

      // KK unik (gabung semua KK)
      ...
      // 2) Tabel lain, hitung count per RW
  ...
  useEffect(() => {
    ...
    // Realtime subscription per tabel
  ...
  return (
    <div className="bg-white rounded-2xl shadow-md border border-slate-200 p-6 flex flex-col h-[480px]">
      <div className="mb-4">
        ...
        <p className="text-sm text-slate-500">Distribusi data per RW (Realtime)</p>
      </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 text-slate-700">
        {pieData.map((entry, index) => (
          <div key={index} className="flex items-center space-x-2">
            <span className="inline-block w-4 h-4 rounded" style={{ backgroundColor: COLORS[index % COLORS.length] }} />
